define(['ko', 'app', 'services/qualityControlService', 'viewModels/qualityControl/labResultDetails', 'viewModels/qualityControl/lotHolds', 'viewModels/shared/lots', 'viewModels/shared/notebooks', 'helpers/koHelpers'],
    function(ko, app, qualityControlService, labResultDetailsModule, lotHolds, lotsBase, notebooks, koHelpers) {
        var microAttributes = {
                "TPC": true,
                "Yeast": true,
                "Mold": true,
        ...
        function init() {
            ...
            self.LabResultDetailsViewModel = labResultDetailsModule;
            self.allDataLoaded = ko.observable(false);

            self.defaultTestDate = ko.observableDate(Date.now());
            self.chileTypeFilter = ko.observable(app.lists.chileTypes.FinishedGoods.key).extend({ chileType: true });
            self.productionStartFilter = ko.observableDate();
            self.productionEndFilter = ko.observableDate();
            self.productionStatusFilter = ko.observable(app.lists.productionStatusTypes.Produced.key).extend({ productionStatusType: true });
            self.qualityStatusFilter = ko.observable(app.lists.lotQualityStatusTypes.Undetermined.key).extend({ lotQualityStatusType: true });
            ...
            self.animateNewItem = koHelpers.animateNewListElement({
            ...
            var lotPager = qualityControlService.getLabResultsPager({
                pageSize: 50,
                parameters: {
            ...
            self.LabResultDetailsViewModel.LabResult.subscribe(function(lotKey) {
                self.displayLoadingMessage(true);
                qualityControlService.getLotByKey(lotKey, {
            ...
            koHelpers.ajaxStatusHelper(self.startingLotFilter);
            koHelpers.ajaxStatusHelper(self.searchLotKeyCommand);
            ...
            function findLotByKeyAsync(lotKey, callbackOptions, bypassLocalCheck) {
                ...
                qualityControlService.getLotByKey(lotKey, callbackOptions);
            }
        ...
        function mapLabResultLotSummary(values) {
            ...
            labResultLot.CustomerName = values.CustomerName;
            labResultLot.CustomerKey = values.CustomerKey;
            
            //computed properties
            labResultLot.requiresQualityControlIntervention = ko.computed(function() {
                return this.QualityStatus() === app.lists.lotQualityStatusTypes.RequiresAttention.key;
            }, labResultLot);
            labResultLot.enableRejection = ko.computed(function() {
                return !this.requiresQualityControlIntervention() && this.QualityStatus() !== app.lists.lotQualityStatusTypes.Rejected.key;
            }, labResultLot);
            labResultLot.enableAcceptance = ko.computed(function() {
                return !this.requiresQualityControlIntervention()
                    && this.QualityStatus() !== app.lists.lotQualityStatusTypes.Released.key
                    && this.QualityStatus() !== app.lists.lotQualityStatusTypes.Contaminated.key;
            }, labResultLot);
            labResultLot.enableQualityHold = ko.computed(function() {
                return !this.requiresQualityControlIntervention() && this.QualityStatus() !== app.lists.lotQualityStatusTypes.RequiresAttention.key;
            }, labResultLot);
        ...
        function updateHistory(value, useReplace) {
            ...
            if (!useReplace && app.navigation.getHashValue() === hash) return;
            ...
            useReplace === true
                ? app.navigation.replaceState.apply(null, args)
                : app.navigation.pushState.apply(null, args);
        }
