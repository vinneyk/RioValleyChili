...
(
    [string]$ServerName,
    [string]$AccessDatabaseName,
    [string]$ProdDatabaseName,
...
Write-Host "`nStarting SQL backup"

Write-Host "Backing up " + $ProdDatabaseName
$ProdBackupFileAndPath = $BackupDirectory + "\" + $ProdDatabaseName + ".bak"
Backup $SQLServer $ProdDatabaseName $ProdBackupFileAndPath | Out-Null

Write-Host "Backing up " + $AccessDatabaseName
$AccessBackupFileAndPath = $BackupDirectory + "\" + $AccessDatabaseName + ".bak"
Backup $SQLServer $AccessDatabaseName $AccessBackupFileAndPath | Out-Null

Write-Host "Compressing backup files..."
CompressFile $ProdBackupFileAndPath ($BackupZipDirectory + "\" + $ProdDatabaseName + ".7z")
CompressFile $AccessBackupFileAndPath ($BackupZipDirectory + "\" + $AccessDatabaseName + ".7z")